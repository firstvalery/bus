...
*/
    ...
    {
		try {
			return Response.status(Response.Status.OK).entity(BusScheduleEngine.getOrder(date)).build();
		} catch (ClassNotFoundException | SQLException e) {
			return Response.status(Response.Status.OK).entity(e.toString()).build();
		}
    }
	...
	@POST
	@ApiOperation(value = "Сгенерировать разнарядку",
	    notes = "Выводится JSON массивом.")
	@ApiResponses(value = { 
		      @ApiResponse(code = 400, message = "Bad Request"),
		      @ApiResponse(code = 404, message = "Not Found") })
	...
	@Produces(MediaType.APPLICATION_JSON)
    public Response generateOrder(String date)
    {
		try {
			return Response.status(Response.Status.OK).entity(BusScheduleEngine.autoGenerateDistribution(date)).build();
		} catch (ClassNotFoundException | SQLException e) {
			return Response.status(Response.Status.OK).entity(e.toString()).build();
		}
    }
	...
	@GET
	@ApiOperation(value = "Получить сведения о генерации",
	    notes = "Выводится JSON массивом.",
	    response = GenerationData.class,
	    responseContainer = "List"
	    )
	@ApiResponses(value = { 
		      @ApiResponse(code = 400, message = "Bad Request"),
		      @ApiResponse(code = 404, message = "Not Found") })
	@Path("/order/generation/status")
	@Produces(MediaType.APPLICATION_JSON)
    public Response getGenerationStatus(
    		@ApiParam(value = "дата начальная", required = true)@QueryParam("date_min") String dateMin,
    		@ApiParam(value = "дата конечная", required = true)@QueryParam("date_max") String dateMax)
    {
		try {
			return Response.status(Response.Status.OK).entity(BusScheduleEngine.getGenerationInfo(dateMin, dateMax)).build();
		} catch (ClassNotFoundException | SQLException e) {
			return Response.status(Response.Status.OK).entity(e.toString()).build();
		}
    }
